...
# vendored Swagger UI bundle (internal/apidocs/swaggerui/, a pinned third-party
# minified library — provenance in that dir's VERSION) is skipped because its
...
added_lines_nodoc=$(echo "$diff" | awk '
  /^diff --git/ { skip = ($0 ~ /\.(md|mdx|txt)( |$)/ || $0 ~ /internal\/apidocs\/swaggerui\//) }
  /^\+/ && $0 !~ /^\+\+\+/ && !skip { print }
')
